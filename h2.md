...
* Mallory cannot maliciously modify the messages, as they are encrypted using the --encrypt and --sign parameters and he does not have the private key of either party, so the digital signature would not match when using the --decrypt parameter.
